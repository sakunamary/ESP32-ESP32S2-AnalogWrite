...
uint8_t Pwm::attachServo(int pin)
{
    uint8_t ch = firstFreeCh();
    if (ch < chMax)
        config_servo(ch, mem[ch].servoMinUs, mem[ch].servoMaxUs);
...
float Pwm::readMicroseconds(int pin)
{
    uint8_t ch = attached(pin);
    if (ch < chMax)
        return mem[ch].duty * ((1000000.0 / mem[ch].frequency) / ((1 << mem[ch].resolution) - 1.0)); // μs
    else
        return 0;
}
    ...
    { // free channels exist
        for (uint8_t c = 0; c < chMax; c++)
        {
            ...
            { // first free ch
                mem[c].pin = pin;
                ch = c;
                if (mem[ch].frequency < 40 || mem[ch].frequency > 900)
                    mem[ch].frequency = 50;
                if (mem[ch].resolution > widthMax)
                    mem[ch].resolution = widthMax;
                else if (mem[ch].resolution < 14 && widthMax == 20)
                    mem[ch].resolution = 16;
                else if (mem[ch].resolution < 14)
                    mem[ch].resolution = 14;
                ledcAttach(ch, mem[ch].frequency, mem[ch].resolution);
                if (sync)
                    pause(ch);
...
void Pwm::tone(int pin, uint32_t frequency, uint16_t duration, uint16_t interval)
{
    ...
    if (ch < chMax)
    {
        ...
        else if (ms - mem[ch].startMs > interval)
            durDone = false;
    }
}
...
void Pwm::note(int pin, note_t note, uint8_t octave, uint16_t duration, uint16_t interval)
{
    ...
    if (octave <= 8 || note <= NOTE_MAX)
        tone(pin, noteFreq, duration, interval);
}

uint8_t Pwm::attached(int pin)
{
    if (!((pinMask >> pin) & 1))
        return 254; // not a pwm pin
    bool freeCh = false;
    for (uint8_t ch = 0; ch < chMax; ch++)
    {
        if (mem[ch].pin == pin)
            return ch;
        if (mem[ch].pin == 255)
            freeCh = true; // free channel(s) exist
    }
...
{
    if ((REG_GET_FIELD(GPIO_PIN_MUX_REG[pin], MCU_SEL)) == 0)
        return true;
    else
        return false;
}
...
void Pwm::pause(int ch)
{
    if (ch == 255)
        ...
        ledc_timer_pause((ledc_mode_t)mem[ch].mode, (ledc_timer_t)mem[ch].timer);
}
...
float Pwm::setFrequency(int pin, uint32_t frequency)
{
    ...
    if (ch < chMax)
    {
        ...
        {
            ledcAttach(ch, frequency, mem[ch].resolution);
            if (sync)
                pause(ch);
...
uint8_t Pwm::setResolution(int pin, uint8_t resolution)
{
    ...
    if (ch < chMax)
    {
        ...
        {
            ledcAttach(ch, mem[ch].frequency, resolution);
            if (sync)
                pause(ch);
            ledcWrite(ch, mem[ch].duty);
...
{
    if (minUs < 500)
        mem[ch].servoMinUs = 500;
    else if (minUs > 2500)
        mem[ch].servoMinUs = 2500;
    else
        mem[ch].servoMinUs = minUs;
    if (maxUs < 500)
        mem[ch].servoMaxUs = 500;
    else if (maxUs > 2500)
        mem[ch].servoMaxUs = 2500;
    else
        mem[ch].servoMaxUs = maxUs;
    if (mem[ch].frequency < 40 || mem[ch].frequency > 900)
        mem[ch].frequency = 50;
    if (mem[ch].resolution > widthMax)
        mem[ch].resolution = widthMax;
...
void Pwm::wr_servo(int pin, float value, double speed, double ke)
{
    ...
    { // write PWM
        ...
        if (value < mem[ch].servoMinUs)
        { // degrees
            if (value < 0)
                value = 0;
            else if (value > 180 && value < 500)
                value = 180;
        ...
        { // microseconds
            if (value < mem[ch].servoMinUs)
                value = mem[ch].servoMinUs;
            else if (value > mem[ch].servoMaxUs)
                value = mem[ch].servoMaxUs;
        ...
        { // easing enabled
            ...
            if (duty > mem[ch].startDuty)
            {
                ...
                if (isnan(mem[ch].ye) || mem[ch].ye < 0.0)
                    mem[ch].ye = 0.0;
                else if (mem[ch].ye > 1.0)
                    mem[ch].ye = 1.0;
